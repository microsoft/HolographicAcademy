...
public class CursorManager : Singleton<CursorManager>
{
    ...
    private static void SetLayerCollisions()
    {
        ...
        for (int i = 0; i < maxLayers; i++)
        {
            ...
            Physics.IgnoreLayerCollision(LayerMask.NameToLayer("Default"), i, true);
        }

        // Ensures the Cursor can collide with the Interactible objects only.
        Physics.IgnoreLayerCollision(LayerMask.NameToLayer("Default"), LayerMask.NameToLayer("Cursor"), false);
    }
    ...
    void Update()
    {
        ...
        // Place the cursor slightly in front of the calculated hit position.
        gameObject.transform.position = GazeManager.Instance.Position + (0.01f * GazeManager.Instance.Normal);

        // Reorient the cursor to match the hit object's normal.
